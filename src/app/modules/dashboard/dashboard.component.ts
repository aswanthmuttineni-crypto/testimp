...
@Component({
  ...
  styles: [`
    ...
    .send-actions { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; justify-content: flex-end; }
    .btn-send { padding: 12px 24px; border-radius: 14px; border: none; background: linear-gradient(135deg,#14b8a6,#0d9488); color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; white-space: nowrap; }
    .btn-send.whatsapp { background: linear-gradient(135deg,#22c55e,#16a34a); }
    ...
    .btn-wa { padding: 5px 10px; border-radius: 8px; border: none; background: #dcfce7; color: #15803d; font-size: 14px; cursor: pointer; }
    ...
    @media (max-width: 768px) { .hero { flex-direction: column; align-items: flex-start; padding: 22px; } .send-actions, .btn-send { width: 100%; } .stats { grid-template-columns: 1fr 1fr; } .snap-grid { grid-template-columns: 1fr 1fr; } }
  `],
  template: `
    <div class="page">
      <div class="hero">
        ...
        <div class="send-actions">
          <button class="btn-send" [disabled]="sending" (click)="sendDueMail()">
            {{ sending && sendingChannel==='EMAIL' ? 'Sending...' : '📧 Send Due Mail' }}
          </button>
          <button class="btn-send whatsapp" [disabled]="sending" (click)="sendDueWhatsApp()">
            {{ sending && sendingChannel==='WHATSAPP' ? 'Sending...' : '📱 Send WhatsApp' }}
          </button>
        </div>
      </div>
      ...
      @if (summary) {
        <div class="stats">
          <div class="scard green"><div class="scard-icon">INR</div><small>Total Income</small><strong>{{ summary.totalIncome | currency:'INR':'symbol':'1.0-0' }}</strong></div>
          <div class="scard red"><div class="scard-icon">EXP</div><small>Total Expenses</small><strong>{{ summary.totalExpenses | currency:'INR':'symbol':'1.0-0' }}</strong></div>
          <div class="scard blue"><div class="scard-icon">NET</div><small>Net Profit</small><strong>{{ summary.profit | currency:'INR':'symbol':'1.0-0' }}</strong></div>
          <div class="scard"><div class="scard-icon">ROOM</div><small>Occupied Rooms</small><strong>{{ summary.occupiedRooms }}</strong></div>
          <div class="scard"><div class="scard-icon">OPEN</div><small>Vacant Rooms</small><strong>{{ summary.vacantRooms }}</strong></div>
          <div class="scard yellow"><div class="scard-icon">DUE</div><small>Pending Rent</small><strong>{{ summary.pendingRent | currency:'INR':'symbol':'1.0-0' }}</strong></div>
        </div>

        <div class="two-col">
          <div class="panel" style="padding:22px;">
            <div class="panel-hdr"><div><p class="eyebrow">Financial Overview</p><h2>Income vs Expenses</h2></div></div>
            <div class="bar-row bar-income"><div class="bar-top"><span>Income</span><strong>{{ summary.totalIncome | currency:'INR':'symbol':'1.0-0' }}</strong></div><div class="bar-track"><span [style.width.%]="pct(summary.totalIncome)"></span></div></div>
            <div class="bar-row bar-expense"><div class="bar-top"><span>Expenses</span><strong>{{ summary.totalExpenses | currency:'INR':'symbol':'1.0-0' }}</strong></div><div class="bar-track"><span [style.width.%]="pct(summary.totalExpenses)"></span></div></div>
            <div class="bar-row bar-profit"><div class="bar-top"><span>Profit</span><strong>{{ summary.profit | currency:'INR':'symbol':'1.0-0' }}</strong></div><div class="bar-track"><span [style.width.%]="pct(summary.profit)"></span></div></div>
          </div>

          <div class="panel" style="padding:22px;">
            <div class="panel-hdr"><div><p class="eyebrow">Quick Snapshot</p><h2>This Month</h2></div></div>
            <div class="snap-grid">
        ...
        <div class="panel" style="padding:22px;">
          ...
          @if (summary.monthlyDues.dues.length) {
            <div class="table-wrap">
              <table>
                <thead>
                  <tr><th>Tenant</th><th>Room / Bed</th><th>Phone</th><th>Email</th><th>Due Amount</th><th>Status</th><th></th></tr>
                </thead>
                <tbody>
                  @for (due of summary.monthlyDues.dues; track due.tenant._id) {
                    <tr>
                      <td><div class="t-cell"><div class="avatar">{{ due.tenant.name.charAt(0).toUpperCase() }}</div><div><strong>{{ due.tenant.name }}</strong><small>{{ due.tenant.email || 'No email' }}</small></div></div></td>
                      ...
                      <td>
                        <button class="btn-wa" [disabled]="sendingWa[due.tenant._id!]" (click)="sendSingleWhatsApp(due.tenant._id!, due.tenant.name)" title="Send WhatsApp to {{ due.tenant.name }}">
...
export class DashboardComponent implements OnInit {
  ...
  sendDueNotification(channel: 'EMAIL' | 'WHATSAPP') {
    ...
    this.notice = channel === 'EMAIL' ? 'Sending due mails...' : 'Sending due WhatsApp messages...';
    ...
    request.subscribe({
      next: res => {
        const result = res as { message: string; sent?: string[]; skipped?: unknown[]; errors?: unknown[] };
        this.sending = false;
  ...
  sendSingleWhatsApp(tenantId: string, tenantName: string) {
    ...
    this.api.notifications.sendMonthlyDueWhatsAppSingle(tenantId).subscribe({
      ...
      error: err => {
        this.sendingWa[tenantId] = false;
        this.notice = err.error?.message || `Failed to send WhatsApp to ${tenantName}`;
        this.noticeType = 'error';
      }
    });
  }
