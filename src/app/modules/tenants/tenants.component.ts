...
//       <h1>Tenants</h1>
//       <p class="page-copy">
//         Keep tenant contact details, room assignment, rent amount, and ID proof
//         in one record.
//       </p>
//     </header>
...
//           <button class="primary" type="button" (click)="openAddTenant()">
//             Add Tenant
//           </button>
//         </div>
//         @if (tenants.length) {
...
//                         <a
//                           [href]="fileUrl(tenant.idProof?.path)"
//                           target="_blank"
//                           >View</a
//                         >
//                       }
...
//                         <button class="secondary" (click)="view(tenant)">
//                           View</button
//                         ><button class="secondary" (click)="edit(tenant)">
//                           Edit</button
//                         ><button class="danger" (click)="remove(tenant)">
//                           Delete
//                         </button>
//                       </div>
//                     </td>
...
//             No tenants added yet. Add tenant details after creating rooms.
//           </div>
//         }
//       </article>
//     </section>
...
//             ><strong>{{
//               selectedTenant.monthlyRent | currency: 'INR' : 'symbol' : '1.0-0'
//             }}</strong>
//           </div>
//           <div>
//             <small>Advance</small
...
//       <div class="modal-backdrop" (click)="closeAddTenant()">
//         <div class="modal" (click)="$event.stopPropagation()">
//           <section class="panel form">
//             <h2>Add Tenant</h2>
//             <form (ngSubmit)="saveAndCloseTenant()">
//               <label
//                 >Name<input [(ngModel)]="form.name" name="modalName" required
//               /></label>
//               <label
//                 >Phone<input
//                   [(ngModel)]="form.phone"
...
//                   [(ngModel)]="form.aadhaarNo"
//                   name="modalAadhaarNo"
//               /></label>
//               <label
//                 >Guardian Name<input
//                   [(ngModel)]="form.guardianName"
//                   name="modalGuardianName"
//               /></label>
//               <label
//                 >Guardian Phone<input
//                   [(ngModel)]="form.guardianPhone"
//                   name="modalGuardianPhone"
//               /></label>
//               <label
//                 >Room<select
//                   [(ngModel)]="form.roomId"
...
//                   required
//                 >
//                   <option [ngValue]="room._id" *ngFor="let room of rooms">
//                     {{ room.roomNo }}
//                   </option>
//                 </select></label
//               >
...
//                 >Joining Date<input
//                   type="date"
//                   [(ngModel)]="form.joiningDate"
//                   name="modalJoiningDate"
//                   required
//               /></label>
//               <label
//                 >Advance<input
//                   type="number"
//                   [(ngModel)]="form.advanceAmount"
//                   name="modalAdvanceAmount"
//               /></label>
//               <label
//                 >Monthly Rent<input
//                   type="number"
//                   [(ngModel)]="form.monthlyRent"
//                   name="modalMonthlyRent"
//                   required
//               /></label>
//               <label
//                 >Status<select [(ngModel)]="form.status" name="modalStatus">
...
//       if (['_id', '__v', 'createdAt', 'updatedAt', 'idProof'].includes(key))
//         return;
//       data.append(key, String(value ?? ''));
//     });
//     if (this.file) data.append('idProof', this.file);
//     const request = this.form._id
//       ? this.api.tenants.update(this.form._id, data)
//       : this.api.tenants.create(data);
//     request.subscribe(() => {
//       this.reset();
//       this.load();
//     });
...
//     if (tenant._id && confirm('Delete this tenant?'))
//       this.api.tenants.delete(tenant._id).subscribe(() => this.load());
//   }
  ...
  template: `
    <section class="tenants-page">
      ...
      <header class="tenants-hero">
        <div>
          <p class="hero-eyebrow">Hostel Tenant Management</p>

          <h1>Tenants</h1>

          <p class="hero-copy">
            Manage tenant records, ID proofs, room allocations, rent details and
            guardian information in one modern dashboard.
          </p>
        </div>
      ...
      <section class="panel">
        <div class="section-title">
          ...
          <div class="tenant-count">{{ tenants.length }} Tenants</div>
        </div>
        ...
        @if (tenants.length) {
          ...
          <div class="empty-state">No tenants added yet.</div>
        }
      </section>
        ...
        <div class="modal-backdrop" (click)="closeAddTenant()">
          <div class="modal" (click)="$event.stopPropagation()">
            <section class="panel form-panel">
              <div class="section-title">
                <div>
                  <p class="eyebrow">Tenant Form</p>

                  <h2>Add Tenant</h2>
                </div>
              </div>

              <form class="tenant-form" (ngSubmit)="saveAndCloseTenant()">
                <div class="form-grid">
                  <label>
                    Name
...
export class TenantsComponent implements OnInit {
  ...
  load() {
    ...
    this.api.tenants.list().subscribe((tenants) => {
